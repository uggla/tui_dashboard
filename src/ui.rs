...
pub fn draw_input(f: &mut ratatui::Frame, app: &App) {
    ...
    let input_line = Line::from(vec![Span::raw(left), Span::styled("|", Style::default()), Span::raw(right)]);
    let title = app.input_title();
    let input = Paragraph::new(input_line).block(Block::default().borders(Borders::ALL).title(title));
...
pub fn draw_timer(f: &mut ratatui::Frame, app: &App) {
    let size = f.area();
    let rows = Layout::default().direction(Direction::Vertical).constraints([Constraint::Length(3), Constraint::Min(3)]).split(size);
    if let Some(conf) = &app.config {
        let header = Paragraph::new(Line::from(vec![
            Span::raw(format!("{} → {} • approach {} min", conf.start.name, conf.destination.name, conf.approach_minutes)),
        ])).block(Block::default().borders(Borders::ALL).title("Config"));
        f.render_widget(header, rows[0]);
    }
    let cols = Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage(60), Constraint::Percentage(40)]).split(rows[1]);
...
pub fn draw_journeys(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let header = Row::new(vec![Cell::from("Date"), Cell::from("Dep"), Cell::from("Arr"), Cell::from("Dur"), Cell::from("Changes")]).style(Style::default().add_modifier(Modifier::BOLD));
    let rows = app.journeys.iter().map(|j| {
        let dur_min = (j.duration_secs / 60).max(0);
        Row::new(vec![Cell::from(j.date_str.clone()), Cell::from(j.dep_hm.clone()), Cell::from(j.arr_hm.clone()), Cell::from(format!("{}m", dur_min)), Cell::from(format!("{}", j.nb_transfers))])
    });
    let table = Table::new(rows, [Constraint::Length(10), Constraint::Length(6), Constraint::Length(6), Constraint::Length(6), Constraint::Length(8)])
        .header(header)
        .block(block)
        .highlight_symbol("▶ ");
...
pub fn centered_rect(percent_x: u16, percent_y: u16, r: Rect) -> Rect {
    ...
    let horizontal = Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage((100 - percent_x) / 2), Constraint::Percentage(percent_x), Constraint::Percentage((100 - percent_x) / 2)]).split(popup_layout[1]);
    horizontal[1]
}
